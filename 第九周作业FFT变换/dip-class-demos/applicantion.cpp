...
/********************************************************
ifft_fselect（） 函数模型：筛选高低频信号
Mat ifft_highf(cv::Mat src，cv::Mat dstimg,int mode );

参数介绍：
		InputArray src：输入数组
		InputOutputArray dst：输出数组，支持原地运算
		mode: 取MASK的模式，0为去除所选区域（用于去除低频信号）
							1为获得所选区域（用于得到低频信号）
********************************************************/
int ifft_fselect(cv::Mat src ,cv::Mat & dstMat,int mode)
{
	cv::Mat dst;
