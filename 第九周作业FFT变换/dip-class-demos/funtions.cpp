...
int removeFrequnce()
{
	...
	selectPolygon(magMat, maskMat,0);
...
/**************************************************************
brief:添加了去除信号模式，MODE=1和MODE=0分别用于去高低频率信号

***************************************************************/
int selectPolygon(cv::Mat srcMat, cv::Mat &dstMat,int mode)
{
	...
	drawContours(m, contours, 0, Scalar(1), -1);
	m.copyTo(dstMat);
